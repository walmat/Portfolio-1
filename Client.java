...
public class Client implements Runnable {
	...
	private boolean recordScore = false;
	private Server s;
	...
	public Client(String ipAddr, String username, String password, String email, Color color_, int serverPort,
			boolean type) {
	...
	public void run() {
		// check for a new message, once we receive it, streamOut will send it
		// to the server
		while (thread != null) {
				...
				if (frame.startRound == true && roundStarted == false) {
					...
					try {
						...
						JOptionPane.showMessageDialog(new JFrame(),
								"Error while trying to start the round" + e.getMessage());
						stop();
					}
				}
	...
	public void handleChat(Object msg) {
		...
		if (msg instanceof Integer) {
			if (answerRound == true) {
				...
				frame.changeBtnSendText(msg + "");
			}

			if ((int) msg <= 0) {
				frame.changeBtnSendText("Send");
				roundStarted = false;
				answerRound = false;
				if (answerInput != null) {
					answerInput.dispose();
			...
			if (recordScore) {
				frame.changeScoreText((String) msg);
				recordScore = false;
			} else {
